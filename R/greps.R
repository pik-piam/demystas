#' Sequential pairwise grep-style matching
#'
#' Sequentially performs a two-way grep-style analysis on two character vectors. Calculates pairwise matching scores based on a rigid customized routine and returns matching strings ranked
...
greps <- function(x, y, sepx = "\\.", sepy = "\\.", limitChar = 0, limitWord = 0, booster = 0.9, wordIgnore = NULL, checkBoth = TRUE, ignore.case = TRUE){
  ...
  rank <- data.frame(matrix(ncol=length(y)+1), stringsAsFactors = FALSE)
    ...
    for(k in 1:length(yList)){
      if(x[i] == y[k] | (all(xList[[i]] %in% yList[[k]]) & all(yList[[k]] %in% xList[[i]]))) {
        ...
        if(measure > limitWord){
          result[i,min(which(is.na(result[i,])))] <- y[k]
          rank[i,min(which(is.na(rank[i,])))] <- measure
        }
      }
    }
    Sys.sleep(1/1000)
  ...
  if(ncol(rank) > 1){
    if(!is.numeric(rank[,-1]) & !is.vector(rank[,-1])){
      ...
      result[,-1] <- sapply(1:nrow(rank), function(i) return(result[i,-1][as.numeric(order(-rank[i,-1]))]))
      rank[,-1] <- sapply(1:nrow(rank), function(i) return(rank[i,-1][as.numeric(order(-rank[i,-1]))]))
    }
  }
